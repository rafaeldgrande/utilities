...
SUBROUTINE elphon()
  ...
  DO irr = 1, nirr
     ...
     !write(*,*) 'OLHA EU dvscfins', dvscfins
     IF (doublegrid) DEALLOCATE (dvscfins)
     !write(*,*) 'OLHA EU dvscfin', dvscfin
     DEALLOCATE (dvscfin)
     IF (okvan) THEN
        DEALLOCATE (int3)
  ...
  !write(*,*) 'TESTE NEW', imode0 + ipert
  !write(*,*) 'TESTE NEW', imode0 + ipert, dvscfin(1,1,ipert)
  !write(*,*) 'TESTE NEW2', imode0 + ipert, dvscfin(:, :, :)
  !
...
SUBROUTINE elphel (irr, npe, imode0, dvscfins)
  ...
  INTEGER :: ibndMINE, jbndMINE, ikMINE, imodeMINE, contaModos
  character (len = 40) :: NAMEMINE
  !
  contaModos = 0
!  write(*,*) 'OLHA AQUI2', irr, npe, imode0
  ...
  DO ik = 1, nksq
     ...
     DO ipert = 1, npe
        ...
        !
        !write(*,*) 'olha 2 dvscfins', dvscfins(:,1,ipert)
        IF ( dffts%has_task_groups ) THEN
           IF (noncolin) THEN
        ...
        DO ibnd = ibnd_fst, ibnd_lst
           DO jbnd = ibnd_fst, ibnd_lst
            ...
            !write(*,*) 'HERE1', ipert + imode0, ik, ibnd, jbnd, elphmat (jbnd, ibnd, ipert)  !!!!! for some reason here is wrong!!!
           ENDDO
        ENDDO
     ENDDO
     !
     ...
     DO ipert = 1, npe
        DO jbnd = ibnd_fst, ibnd_lst
           DO ibnd = ibnd_fst, ibnd_lst
              ...
              !write(*,*) 'HERE2', ipert + imode0, ik, ibnd, jbnd, el_ph_mat (ibnd, jbnd, ik, ipert + imode0)  !!!!!
           ENDDO
        ENDDO
     ENDDO
  ENDDO
  ...
  if(elph_tetra == 0 .AND. .NOT. elph_ahc) then
     ...
     !write(*,*) 'OLHA AQUI', irr, npe, imode0
!     write(1294, *) 'teste, irr, npe, imode0', irr, npe, imode0
     DO imodeMINE = 1, npe
      !write(*,*) 'OLHA AQUI', irr, contaModos, irr + contaModos - 1, imodeMINE + imode0
      write(NAMEMINE, '(I1)') imodeMINE + imode0
      open(1294, file='dfpt_pre.dat'//NAMEMINE)
      DO ikMINE = 1, nksqtot
         DO ibndMINE = ibnd_fst, ibnd_lst
            DO jbndMINE = ibnd_fst, ibnd_lst
               write(1294, *) ikMINE, ibndMINE, jbndMINE, el_ph_mat_rec_col(ibndMINE, jbndMINE, ikMINE, imodeMINE)
            ENDDO
         ENDDO
      ENDDO
      close(1294)
